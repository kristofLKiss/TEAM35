...
xlname = f"results_{maxh_str}_{now_str}_Current.xlsx"
...
class CoilOptimizationProblem(Problem):
    def __init__(self):
        super().__init__(n_var=n_turns, n_obj=2, n_constr=0, xl=mincurrent, xu=maxcurrent)

    def _evaluate(self, x, out, *args, **kwargs):
        f1_values = []
        f2_values = []
        for I in x:
            [_, _, _, f1, f2, _, _, _, _, _] = calc_with_ROM_source_optimization(mesh, mesh_plus, mesh_minus, RomAred, RomAredPlus, RomAredMinus, FEspace_red, FEspace_redPlus, FEspace_redMinus, I, radius, coordinates, B_ref,n_turns,w,h,sigma_coil)
            f1_values.append(f1)
            f2_values.append(f2)
...
for I in pareto_currents:
    ...
    Br_valsFEM, Bz_valsFEM, B_valsFEM, f1FEM, f2FEM, _, _, _, _, _, _ = fem_simulation_sourceopt(
    ...
    Br_valsROM, Bz_valsROM, B_valsROM, f1ROM, f2ROM, _, _, _, _, _ = calc_with_ROM_source_optimization(mesh, mesh_plus, mesh_minus, RomAred, RomAredPlus, RomAredMinus, FEspace_red, FEspace_redPlus, FEspace_redMinus, I, radius, coordinates, B_ref,n_turns,w,h,sigma_coil)
